...
public class Car {
	...
	*/
	public void fillingFuel(Fuel t_fuel, int amount_in_liter) throws FuelException {
		if (getFuel() != t_fuel) throw new FuelException(t_fuel);
		else System.out.println("Berhasil isi bensin sebanyak " + Integer.toString(amount_in_liter) + " liter");
	}
	...
	*/
	public void changeGear(int t_gear) throws InvalidInputCarException {
		if (t_gear >= MIN_GEAR && t_gear <= MAX_GEAR) {
			gear = t_gear;
			System.out.println("Berhasil mengubah gigi menjadi gigi " + Integer.toString(t_gear));
		} else {
	...
	*/
	public void changeAcPower(int t_ac_power) throws InvalidInputCarException {
		if (t_ac_power >= MIN_AC && t_ac_power <= MAX_AC) {
			ac_power = t_ac_power;
			System.out.println("Berhasil mengubah power ac menjadi " + Integer.toString(t_ac_power));
		} else {
